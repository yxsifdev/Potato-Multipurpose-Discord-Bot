...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (!usuario)
      return message.reply({ content: `${emj.deny} Especificar un usuario` });
    if (usuario.user.id === message.author.id)
      return message.reply({ content: `${emj.deny} No puedes sancionarte` });
    if (usuario.user.bot)
    ...
    if (usuario.id === message.guild.ownerId)
      return message.reply({
        content: `${emj.deny} No puedes sancionar al dueño del servidor`,
      });
    if (usuario.permissions.has("Administrator"))
      return message.reply({
        content: `${emj.deny} No puedes sancionar a un administrador`,
      });
