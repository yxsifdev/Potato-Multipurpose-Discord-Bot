...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (usuario.user.id === message.author.id)
      return message.reply({
        content: `${emj.deny} No puedes sancionarte a ti mismo`,
      });

    if (usuario.user.bot)
      return message.reply({
        content: `${emj.deny} No puedes sancionar a un bot`,
      });

    if (usuario.roles.highest.position >= message.member.roles.highest.position)
      return message.reply({
        content: `${emj.deny} No puedes sancionar a un miembro con un rol más alto que el tuyo`,
      });

    if (!args[1])
      return message.reply({
        content: `${emj.deny} Especificar una razón para la advertencia`,
      });
    ...
    try {
      ...
      const userWarnings = warnData.warnings.filter(
        (warning) => warning.userId === usuario.user.id
      );
    //   if (userWarnings.length >= 5) {
    //     return message.reply({
    //       content: `${emj.deny} ${usuario.user} ya ha alcanzado el máximo de advertencias permitidas.`,
    //     });
    //   }
      ...
      await message.channel.send({
        content: `${emj.check} \`#${warnData.caseCounter}\` ${usuario.user} Ha sido advertido`,
      });
    } catch (error) {
      console.error(error);
      await message.reply({
        content: `${emj.deny} Hubo un error al aplicar la advertencia.`,
      });
    }
  },
};
