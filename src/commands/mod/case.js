  ...
  async execute(message, args, client) {
    ...
    if (isNaN(caseId))
      return message.reply({
        content: `${emj.deny} Especificar un ID de caso válido.`,
      });
    ...
    try {
      ...
      const caseInfo = userLogs.warnings.find(
        (warning) => warning.case === caseId
      );
      ...
      console.error(error);
      await message.reply({
        content: `${emj.deny} Hubo un error al recuperar la información del caso.`,
      });
    }
  },
};
